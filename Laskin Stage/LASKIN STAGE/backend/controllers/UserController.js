    ...
    try {
        ...
        if (Usuario) {
            ...
            if (passwordIsValid) {
                ...
                console.log('Usuario autenticado con éxito.'); // Registro de éxito
                res.status(200).send({
                    message: 'Bienvenido',
                    token: token,
                    user:
                ...
                console.log('Error en la autenticación: Contraseña incorrecta.'); // Registro de error
                res.status(401).json({
                    error: 'Error en el usuario o contraseña.'
                });
            }
        } else {
            console.log('Error en la autenticación: Usuario no encontrado.'); // Registro de error
            res.status(404).json({
                error: 'Error en el usuario o contraseña.'
            });
        }
    } catch (error) {
        console.error('Error en el servidor:', error); // Registro de error
        res.status(500).send({
            error: '¡Error en el servidor!'
        });
        next(error);
