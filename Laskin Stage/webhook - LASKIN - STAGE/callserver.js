...
async function createorder(order) {
  ...
  try {
    ...
    if (order.discount_applications && order.discount_applications.length > 0 && (order.discount_applications[0].type === 'automatic' || order.discount_applications[0].description === 'Bundle discount' || order.discount_codes[0].code.startsWith('FX'))) {
    ...
    console.log('VALOR', typeof state.value);
...
async function productsHW() {
  const url = 'https://histoweb.co/laskinstage.rest/api/productspricelist';
  const plaintext = process.env.TOKEN;
        ...
        if (stockValue) {
          ...
          const transformedProducts = responseBody.map((product) => {
            ...
            return {
              ...
              vendor: product.product_brand ? product.product_brand.toUpperCase() : "",
              status: "draft",
              template_suffix: "",
              published: false,
...
async function proceduresHW() {
  const url = 'https://histoweb.co/laskinstage.rest/api/procedurespricelist';
  const plaintext = process.env.TOKEN;
  const encryptedText = getEncryptedText(plaintext);
...
async function procedures() {
  ...
  try {
    ...
    return error('Error al consultar los productos:', error);
  }
};
